...
function ParseDocument(document, grammar) {
    ...
    for (let i = 0; i < document.lineCount; i++) {
        ...
        if ((oneString[0] !== -1 && oneString[1] !== -1)) {
            ...
            let oneSymbol = new MySymbol(resultItem, oneString[0], oneString[1]);
            mySymbols.push(oneSymbol);
        }
        // 3- DB fields
        // find DB fields like you do with strings,  
        var oneDB = findSymbol(oneLine, '{', '}');
        if (oneDB[0] !== -1 && oneDB[1] !== -1) {
            ...
            if (!inside) {
                ...
                let oneSymbol = new MySymbol(resultSymbol, oneDB[0], oneDB[1]);
                mySymbols.push(oneSymbol);
            }
        }
        //4 - Variables
        ...
        while (match = pattern.exec(oneLine)) {
            var word = oneLine.slice(startofword, match.index);
            ...
            for (let node of grammar.repository.keywords.patterns) {
                ...
                if (node["match"].match(replace)) {
                    var newMatch = node["match"].replace(replace, "");
                }
            ...
            if (word.length > 0 && word.match(/^[a-zA-Z]/) && !isKeyword) { // and exclude = false:
                ...
                if (!inside) {
                    ...
                    //symbols.push(resultSymbol);
                    // also addit to mysms array
                    let oneSymbol = new MySymbol(resultSymbol, startofword, match.index);
                    mySymbols.push(oneSymbol);
                }
            }
...
class oneWord {
    constructor(pWord, pStart, pEnd) {
    }
}
function findSymbol(pLine, pDelimOpen, pDelimClose) {
...
function parseForDBFieldEnd(pStatus) {
    // check start DBField
    let DBFieldStart = 0;
...
function parseForStringEnd(pStatus) {
    let endQuote = -1;
...
function RemoveBracketFromName(pName) {
